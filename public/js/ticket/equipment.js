...
$('#addEquipment').click(function() {
    ...
    $('#item-'+counter).on('change', function (e) {
        ...
        if (value>=1) {
            ...
            $('#sub-' + id).empty();
            $('#sub-' + id).append($("<option disabled selected>").attr('value', 0).text('Equipment Label'));
            ...
            for (var i = 0; i < rates.length; i++) {
                if (rates[i].category_id == 4 && rates[i].asset_id == value && rates[i].client_id == $('#client_id option:selected').val()) {
                    switch (rates[i].type) {
                        case 1:
