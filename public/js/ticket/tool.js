...
$('#addTool').click(function() {
    ...
    $('#item-'+counter).on('change', function (e) {
        ...
        $('#sub-'+id).empty();
        $('#sub-'+id).append($("<option disabled selected>").attr('value', 0).text('Tool'));
        ...
        for (var i = 0; i < rates.length; i++) {
            if (rates[i].category_id == 5 && rates[i].asset_id == value && rates[i].client_id == $('#client_id option:selected').val()) {
                switch (rates[i].type) {
                    case 1:
