...
$('#addTrailer').click(function() {
    addSelection();
    $('#item-'+counter).append($("<option disabled selected>").attr('value', 0).text('Trailer Type'));
            for (var i = 0; i < assets.length; i++) {
                if (assets[i]['category_id'] == 3)
                    $('#item-'+counter).append($("<option>")
                        .attr('value', assets[i]['id']).text(assets[i]['name']));
            }
            $('#cat-'+counter).attr("value", 3);
    ...
    $('#item-'+counter).on('change', function (e) {
        ...
        $('#sub-'+id).empty();
        $('#sub-'+id).append($("<option disabled selected>").attr('value', 0).text('Trailer Label'));
        ...
        for (var i = 0; i < rates.length; i++) {
            if (rates[i].category_id == 3 && rates[i].asset_id == value && rates[i].client_id == $('#client_id option:selected').val()) {
                switch (rates[i].type) {
                    case 1:
